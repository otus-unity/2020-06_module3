...
public class PlayerIK : MonoBehaviour
{
    ...
    void FootIK(AvatarIKGoal goal, Transform toeBase, string weightName)
    {
        ...
        RaycastHit hit;
        Ray ray = new Ray(footPosition + Vector3.up * MaxDistanceBelowGround, Vector3.down);
        if (Physics.Raycast(ray, out hit, MaxDistanceBelowGround + MaxDistanceAboveGround, layerMask)) {
