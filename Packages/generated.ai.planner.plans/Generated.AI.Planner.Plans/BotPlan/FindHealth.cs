...
{
    [BurstCompile]
    struct FindHealth : IJobParallelForDefer
    {
        ...
        internal FindHealth(Guid guid, NativeList<StateEntityKey> statesToExpand, StateDataContext stateDataContext)
        {
            ActionGuid = guid;
        ...
        public void Execute(int jobIndex)
        {
            ...
            var transitionInfo = new NativeArray<FindHealthFixupReference>(argumentPermutations.Length, Allocator.Temp);
            for (var i = 0; i < argumentPermutations.Length; i++)
            {
                transitionInfo[i] = new FindHealthFixupReference { TransitionInfo = ApplyEffects(argumentPermutations[i], stateEntityKey) };
            }

            // fixups
            var stateEntity = stateEntityKey.Entity;
            var fixupBuffer = m_StateDataContext.EntityCommandBuffer.AddBuffer<FindHealthFixupReference>(jobIndex, stateEntity);
    ...
    public struct FindHealthFixupReference : IBufferElementData
    {
